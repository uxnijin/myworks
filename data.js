...
const DESIGNS = [
  ...
  },
  {
    ...
    lede: 'Kerala has more psychiatrists per person than any other Indian state, and still around 83% of people with a mental-health condition get no treatment. The wall isn\'t supply - it\'s stigma, shared phones, and not knowing what happens in a first session. Oppam (ഒപ്പം - "beside you") is a concept redesign of a real therapy service that designs the minutes before the booking, and builds the therapist\'s side of the app too. A runnable iOS prototype, captured screen by screen.',
  ...
  },
  {
    ...
    lede: 'KSRTC runs 5,576 buses out of 93 depots, and its booking platform is where most of Kerala meets it. It works — but it asks the passenger to do the carrying: a page per decision, a fare with no breakdown, a refund you only learn after you cancel. This is a full redesign, re-architected around one continuous booking surface, built on Uber\'s Base design system in SwiftUI, and shipped as a runnable iOS app rather than a Figma file.',
  ...
  },
  {
    ...
    lede: 'An operations team moves thousands of parcels a day across carriers that each have their own portal, their own tracking format and their own idea of what “delayed” means. So the real job is triage — find the ones going wrong before a customer calls. ShipFlow opens on that: metric cards that are themselves filters, one status vocabulary across every carrier, and every unhappy state drawn on purpose. Designed and built end-to-end with no framework and no build step.',
